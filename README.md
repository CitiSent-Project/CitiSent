...
*Runs on `http://localhost:4000` (API prefix: `/api/v1`).*
...
*Opens the Expo CLI. Press `a` for Android Emulator, `i` for iOS Simulator, `w` for Web, or scan the QR code using the **Expo Go** app on your physical mobile device.*
...
*Runs FastAPI on `http://localhost:8000` (Interactive API docs at `http://localhost:8000/docs`).*
