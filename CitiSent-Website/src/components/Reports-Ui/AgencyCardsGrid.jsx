      ...
      {items.map((item) => {
        ...
        <button
          ...
          className={`w-full rounded-xl px-4 py-3 text-left font-medium transition-all duration-200
                  bg-blue-500 text-white shadow-md hover:shadow-lg active:scale-95 active:shadow-inner 
                    focus:outline-none focus:ring-2 focus:ring-blue-800
                    ${selectedItemId === item.id ? "border-2 border-blue-800" : "border-2 border-transparent hover:border-blue-800"
          }`}
          aria-pressed={selectedItemId === item.id}
        >
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-full border border-slate-300/60 bg-white/70 text-slate-700">
              {iconSrc ? (
                <img
                  src={iconSrc}
                  alt={`${item.label} icon`}
