...
export function AuthPageShell({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`mb-6 ${isFigmaLogin ? "flex justify-center" : "flex items-center gap-3"}`}
        >
          <div
            className={`flex items-center ${isFigmaLogin ? "gap-4 md:gap-5" : "gap-3"}`}
          >
            <p
              className={`text-2xl font-bold ${isFigmaLogin ? "text-[#1f3f73] md:text-[52px]" : "text-slate-900"}`}
            >
              CitiSent
            </p>
            <img
              src={CitiSentLogo}
              alt="CitiSent"
              className={`${isFigmaLogin ? "h-19 w-19 md:h-21 md:w-21" : "h-13 w-13"}`}
            />
          </div>
        ...
        <header className={`mb-6 ${isFigmaLogin ? "mt-2 md:mt-4" : ""}`}>
          <h1
            className={`font-semibold ${isFigmaLogin ? "text-5xl text-white md:text-[54px]" : "text-3xl text-slate-900"}`}
          >
            {title}
          </h1>
          <p
            className={`mt-1 text-sm ${isFigmaLogin ? "text-white/85" : "text-slate-600"}`}
          >
            {subtitle}
          </p>
        </header>
        ...
        {footer ? (
          <footer
            className={`mt-6 text-sm ${isFigmaLogin ? "text-white/90" : "text-slate-600"}`}
          >
            {footer}
          </footer>
        ) : null}
