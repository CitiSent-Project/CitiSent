    ...
    <div className="space-y-4">
      ...
      >
        <form className="space-y-3" onSubmit={handleSubmit}>
          <div>
            <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">Requested department</label>
            <DropdownButton
              className="w-full"
          ...
          <div>
            <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">Reason</label>
            <textarea
              rows={3}
              value={reason}
              onChange={(event) => setReason(event.target.value)}
              placeholder="Describe why the transfer is needed."
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-800 dark:border-slate-600 dark:bg-slate-900/50 dark:text-slate-100"
            />
          </div>
          <button
            type="submit"
            className="rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-500"
          >
            Submit request
          </button>
        </form>
        ...
        {ownRequests.length === 0 ? (
          ...
          <div className="space-y-3">
            {ownRequests.map((request) => (
              <div key={request.id} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
                ...
                <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{request.reason}</p>
                {request.reviewNotes ? (
                  <p className="mt-2 text-xs text-slate-600 dark:text-slate-400">Review notes: {request.reviewNotes}</p>
                ) : null}
              </div>
            ))}
          </div>
