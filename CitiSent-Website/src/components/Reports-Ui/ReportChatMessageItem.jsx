...
export function ReportChatMessageItem({ message, isAdmin }) {
  ...
  return (
    <div className={`flex min-w-0 max-w-full ${isAdmin ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[85%] sm:max-w-[75%] min-w-0 rounded-2xl px-4 py-3 shadow-sm ${isAdmin ? 'rounded-br-sm bg-blue-700 text-white' : 'rounded-bl-sm border border-slate-200/90 bg-white text-slate-800'}`}>
        ...
        <p className={`mt-1.5 flex items-center justify-end gap-1 text-[10px] ${isAdmin ? 'text-blue-100' : 'text-slate-400'}`}>
